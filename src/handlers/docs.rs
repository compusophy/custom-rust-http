...
use crate::types::{ApiDocs, ApiEndpoint};

pub async fn api_docs_json() -> Json<ApiDocs> {
    Json(ApiDocs {
...
pub async fn api_docs_html() -> Html<String> {
    ...
    if !database_endpoints.is_empty() {
        ...
        for endpoint in &database_endpoints {
            let id = format!(
                ...
                endpoint.path.replace("/", "-").replace("api-", "")
            );
            sidebar_items.push_str(&format!(
                "<a href=\"#{}\" class=\"nav-item\">{} {}</a>",
    ...
    fn format_endpoint_card(endpoint: &ApiEndpoint, id: &str) -> String {
        ...
        let performance_html = endpoint
            .performance
            .as_ref()
            .map(|p| format!(r#"<div class="performance">⚡ {}</div>"#, p))
            .unwrap_or_default();

        let request_html = endpoint
            .example_request
            .as_ref()
            .map(|r| {
                format!(
